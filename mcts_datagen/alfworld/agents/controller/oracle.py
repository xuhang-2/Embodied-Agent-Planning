    ...
    def step(self, action_str):
        ...
        try:
            ...
            if cmd['action'] == self.Action.GOTO:
                ...
                # changed by hang
                if event.metadata['errorMessage'] == 'No valid positions to place object found':
                    event.metadata['lastActionSucces'] = True
                    for a_obj in self.env.last_event.metadata['objects']:
                        if a_obj['objectId'] == recep['object_id']:
                            a_obj['receptacleObjectIds'].append(self.env.last_event.metadata['inventoryObjects'][0]['objectId'])
                ...
                # changed by Hang
                if event and not event.metadata['lastActionSuccess'] and 'DeskLamp' in obj['object_id']:
                    event.metadata['lastActionSuccess'] = True
                    # update 
                    for a_obj in self.env.last_event.metadata['objects']:
                        if a_obj['objectId'] == obj['object_id']:
                            a_obj['visible'] = True
                            a_obj['isToggled'] = True
                ...
                # changed by Hang
                self.feedback = "You clean the %s using the %s." % (obj, tar)
                self.env.cleaned_objects = self.env.cleaned_objects | set([object['objectId']]) if object['objectId'] is not None else set()
